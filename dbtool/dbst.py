...
def querydb(query="SELECT * FROM default.diamonds LIMIT 2"):
    with sql.connect(
        ...
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS money (code string, people string, target string, amount float)")
...
def insertData(code, name, target, amount):
    with sql.connect(
        ...
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS money (code string, people string, target string, amount DECIMAL(5,2))")
            values = ",".join(["\"" + key + "\"" for key in [code, name, target]] + [str(amount)])
            cursor.execute(f"INSERT INTO money (code, people, target, amount) VALUES ({values})")
